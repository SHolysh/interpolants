...
cols <- c("standard temperature"="black","local temperature range"="black")
ggplot(dat, aes(x)) +
  theme(legend.title = element_blank(), legend.position = c(0.8, 0.8), legend.background = element_blank()) +
  geom_ribbon(aes(ymin = plow, ymax = phgh, fill="local temperature range"), alpha=.4) +
  ...
  labs(x="metres above sealevel (masl)", 
       ...
       title="Barometric pressure with elevation \nwhen P=101.3 kPa at sea-level") +
