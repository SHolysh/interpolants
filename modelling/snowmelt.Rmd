...
The model represents the snowpack as a single layer and represents processes at the point scale that does not consider spatial
...
By accounting for the snowpack's mass states, the balance between liquid and ice is varying constantly. Eventually, as the winter ends, the balance heavily favours the liquid form and the pack melts away.
...
The model parameters ($C_\text{DDF}=0.021\text{ m}°\text{C}^{-1}\text{d}^{-1}$, $C_\text{CCF}=0.000055$, $C_\text{dens}=0.020$, $C_\text{TSF}=0.049$ and $T_b=1.33°\text{C}$) were optimized globally (against all stations simultaneusly) using the shuffled complex evolution scheme (Duan et.al., 1993---[using this code](https://github.com/maseology/glbopt)). 
