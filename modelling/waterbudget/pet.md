...
Future models may also incorperate more sophisticated methods such as the combination approaches. These methods require knowledge of net radiation ($Q^*=K^*+L^*$), where $K^*=(1-\alpha)K_\text{in}$ is the net shortwave radiaton, $\alpha$ is the surface albedo, and $L^*$ is the net long-wave radiation that can be approximated by (Budyko, 1974; Nov\'ak, 2012):
